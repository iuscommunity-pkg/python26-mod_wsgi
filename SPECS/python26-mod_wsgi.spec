...
%posttrans
# hack for previous ius/rackspace mod_wsgi-python26 installs
if [ -e '/etc/httpd/conf.d/wsgi.conf.rpmsave' ]; then
    mv  /etc/httpd/conf.d/python26-mod_wsgi.conf \
        /etc/httpd/conf.d/python26-mod_wsgi.conf.rpmnew
    mv  /etc/httpd/conf.d/wsgi.conf.rpmsave \
        /etc/httpd/conf.d/python26-mod_wsgi.conf

    sed  --in-place=".bak" "s/mod_wsgi.so/python26-mod_wsgi.so/g" \
        /etc/httpd/conf.d/python26-mod_wsgi.conf
...
%changelog
* Fri Oct 10 2010 BJ Dierkes <wdierkes@rackspace.com> 3.3-2.ius
- Add hacks for upgrading from ius mod_wsgi-python26
